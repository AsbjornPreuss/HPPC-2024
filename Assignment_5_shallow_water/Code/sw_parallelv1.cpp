...
void integrate(Water &w, const real_t dt, const real_t dx, const real_t dy, const real_t g) {
    ...
    #pragma acc parallel loop collapse(2) num_gangs(number_of_gangs_in_run) vector_length(vector_lengths)//present(w.e[0:NX][0:NY],w.u[0:NX][0:NY],w.v[0:NX][0:NY], dt, dx, dy, g)
...
void simulate(const Sim_Configuration config) {
    ...
    std::cout << " 1 " << std::accumulate(water_world.e.front().begin(), water_world.e.back().end(), 0.0) << " ";
