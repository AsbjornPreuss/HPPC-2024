...
void exchange_vertical_ghost_lines(grid_t& data0,grid_t& data1) {
    #pragma acc parallel loop gang num_gangs(number_of_gangs_in_run) vector_length(vector_lengths)// present(data[0:NX][0:NY], NX, NY)
...
void integrate(Water &w, const real_t dt, const real_t dx, const real_t dy, const real_t g) {
    ...
    #pragma acc parallel loop gang vector collapse(2) present(w) num_gangs(number_of_gangs_in_run) vector_length(vector_lengths)
    ...
    #pragma acc parallel loop gang vector collapse(2) num_gangs(number_of_gangs_in_run) vector_length(vector_lengths) //present(w.e[0:NX][0:NY],w.u[0:NX][0:NY],w.v[0:NX][0:NY], dt, dx, dy, g)
...
void simulate(const Sim_Configuration config) {
    ...
    //const real_t dtdx = dt/dx; const real_t dtdy
    #pragma acc data copyin(water_world, dt,  dx, dy, g, NX, NY)//water_world.e[0:NX][0:NY],water_world.u[0:NX][0:NY],water_world.v[0:NX][0:NY],
    {
    for (uint64_t t = 0; t < config.iter; ++t) {
        integrate(water_world, dt, dx, dy, g);
    ...
    std::cout << " 2 " << std::accumulate(water_world.e.front().begin(), water_world.e.back().end(), 0.0)<< " ";
