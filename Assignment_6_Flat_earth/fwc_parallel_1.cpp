...
void exchange_ghost_cells(World &world) {
    ...
    ////// If we only run on one rank, then just do the old thing, no need to have it send stuff to itself and all that.
    if(mpi_size==1)
    {
...
void simulate(uint64_t num_of_iterations, const std::string& model_filename, const std::string& output_filename) {
    ...
    ////// World is not a type that is simple to broadcast, unless we turn it into an MPI_type. May be worth looking at but not as a first priority probably.
    ...
    //////These are the global sizes for the system
    const long int offset_longitude_global = -1; // -1 because first cell is a ghostcell
    const long int offset_latitude_global  = -1;
    const uint64_t longitude_global = global_world.longitude + 2; // one ghost cell on each end
    const uint64_t latitude_global  = global_world.latitude  + 2;
    ////// These are new. Number of ranks in x and y direction. As a first go, same number in each direction, but could also be making each section as square-like as possible, to minimise number of ghost cells.
    ////// With this setup number of ranks must be a square number
    const long int num_ranks_x = int(sqrt(mpi_size));
    const long int num_ranks_y = int(sqrt(mpi_size));

    ////// Local to the ranks
    //// Where in the rank grid is this rank
    const long int rank_position_x = mpi_rank % num_ranks_x;
    const long int rank_position_y = mpi_rank / num_ranks_y;

    //// How is the rank's grid offset.    
    const long int offset_longitude = (longitude_global / num_ranks_x) * rank_position_x  - 1;
    const long int offset_latitude  = (latitude_global  / num_ranks_y) * rank_position_y  - 1;
    const long int end_position_longitude = (longitude_global / num_ranks_x) * (rank_position_x + 1)  + 1;
    const long int end_position_latitude  = (latitude_global  / num_ranks_y) * (rank_position_y + 1)  + 1;

    //// How large is this ranks local grid
    const long int longitude = end_position_longitude - offset_longitude;
    const long int latitude  = end_position_latitude -  offset_latitude;

    
    // copy over albedo data to local world data
    ////// This one is, I think, already good
    ...
    ////// Creates Local part of world with the albedo array defined above, and with and array "data" containing temperature, which is initially constant everywhere at 293.15 
    ...
    ////// Here we set up MPI_types for the horizontal and vertical exchanges. We have no corners so vertical exchange can be contiguous type, and horizontal exchange can be vector type
    //// Type for horizontal exchange. This is not contiguous, so we use a vector type and pick up one element each time
    MPI_Datatype horizontal_exchange_type;
    MPI_Type_vector(longitude, //count, how many strided chunks to include
                    1,         //blocklength, how many elements to include
                    latitude,  //Stride, length of both included and jumped over elements in each chunk
                    MPI_DOUBLE,//Old_type, type of data that is getting packed
                    &horizontal_exchange_type);//new_type, name of the new MPI_Type to create
    MPI_Type_commit(&horizontal_exchange_type);
    ...
    //// Free up the custon MPI_Types again
    MPI_Type_free(&horizontal_exchange_type);
}
...
int main(int argc, char **argv) {
    ...
    // Checks that all input arguments are valid and that no necessary ones are missing
